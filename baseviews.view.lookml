...
- explore: base
  ...
  joins:
    ...
    - join: programs
      fields: [ref_agency, name, project_type_code, funder, agency_project_name, id, list_of_program_names, added_date, description, geocode, count]
    ...
    - join: static_demographics
      ...
      view_label: 'Clients'
      type: inner
      fields: [id, gender, gender_text, ethnicity, ethnicity_text, name_middle, ref_client, race , race_text, veteran, veteran_text, zipcode]
...
- explore: clients
  ...
  joins:
    ...
    - join: static_demographics
      from: client_demographics
      type: inner
      fields: [id, gender, gender_text, ethnicity, ethnicity_text, ref_client, race , race_text, veteran, veteran_text, zipcode]
      sql_on: ${clients.id} = ${static_demographics.ref_client}
...
#       sql_on:  ${client.id} = ${static_demographics_custom.ref_client}
    ...
    - join: referrals
      sql_on: ${client.id} = ${referrals.ref_client}  and ${client_assessments.id} = ${referrals.ref_assessment}
...
#       sql_on: ${referrals.ref_agency} = ${referring_agencies.id}  
    ...
    - join: referto_program
      from: programs
      fields: [ref_agency, name, project_type_code, funder, id,  list_of_program_names, added_date, description, geocode, count]
...
#   from: agencies
# #  fields: [id, coc, name, county]
#   label: 'Project Descriptor - Project Services Only Model'
# #  access_filter_fields: [agencies.id]
#   always_join: [programs]
#   sql_always_where: (programs.deleted = 0 or programs.deleted is null) and agency_info.status = 1
#   joins:
# 
#     - join: counties
#       fields: []
#       sql_on: ${counties.id} = ${agency_info.ref_county}  
#       
#     - join: programs
#       from: programs
#       relationship: one_to_many
#       sql_on: ${agency_info.id} = ${programs.ref_agency}   
# 
#     - join: program_services
#       fields: []
#       sql_on: ${programs.id} = ${program_services.ref_program}  
# 
#     - join: agency_services
#       from: agency_services
#       sql_on: ${program_services.ref_service} = ${agency_services.id}  
#     
# 
#     - join: service_items
#       fields: []
...
#       
#     - join: members
#       sql_on: ${agency_info.id} = ${members.ref_agency} 
#     
#     - join: users
#       fields: []
#       sql_on: ${users.id} = ${members.ref_user}     
